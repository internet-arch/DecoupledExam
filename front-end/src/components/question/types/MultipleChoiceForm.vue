...
const onSubmit = () => {
  ...
  if (correctOptions.length === 0) return alert('请至少选择一个正确答案')

  const total = options.length
  // 生成 "A,C,D" 格式的字符串
  const correctLabels = options
      .map((opt, i) => opt.isCorrect ? labels[i] : null)
      .filter(Boolean)
      .join(',')
  ...
  const payload: SubmitPayload = {
    ...
    questionComponents: [
      ...
      },
      {
        ...
        content: JSON.stringify({
          correctOption: correctLabels, // "A,C,D"
