  ...
  <div class="space-y-6">
    ...
    <div>
      ...
      <textarea
          ...
          rows="5"
          placeholder="例如：Java中的String类是不可变的。"
      ></textarea>
    ...
    <div>
      ...
      <div class="flex gap-4">
        <label class="cursor-pointer label flex items-center gap-2">
          ...
          />
          <span class="label-text text-base">正确</span>
        </label>
        <label class="cursor-pointer label flex items-center gap-2">
          <input
              v-model="formData.correctResult"
              ...
              class="radio radio-primary"
          />
          <span class="label-text text-base">错误</span>
        </label>
      </div>
    </div>
...
const formData = ref({
  ...
  difficulty: 1,
  subjectId: 4,
  correctResult: true, // true = 正确，false = 错误
  answerDesc: '',
  analysisDesc: ''
})
...
// ====== Submit Logic ======
const onSubmit = () => {
  const { title, difficulty, subjectId, correctResult, answerDesc, analysisDesc } = formData.value
    ...
    questionComponents: [
      {
        ...
        content: JSON.stringify({
          correctResult, // boolean: true / false
